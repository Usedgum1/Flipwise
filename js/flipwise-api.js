...
(function() {
  ...
  var TIMESERIES_BASE = 'https://prices.runescape.wiki/api/v2/osrs';
  ...
  function fetchParallel() {
    return Promise.all([
      ...
      return { prices: arr[0], mapping: mapping, buyLimits: buyLimits, iconById: iconById, idByName: idByName };
    });
  }
  ...
  function processMoneyMakers(prices, iconById, buyLimits) {
    ...
    var tiles = [];
    function addTile(key, title, profit, unit, breakdown, itemName) {
      var label = profit > 0 ? '+' + Number(profit).toLocaleString() + ' gp' : (profit < 0 ? Number(profit).toLocaleString() + ' gp' : 'Inactive');
      var t = { key: key, title: title, profit: profit, label: label, unit: unit || 'gp', tooltip: tooltips[key] || '', itemName: itemName || null };
    ...
    function geSellValue(d) {
      d = d || {};
      return Math.max(d.high || 0, d.low || 0);
    }
    function geTaxEach(value) {
      return value ? Math.min(Math.floor(value * 0.02), maxTax) : 0;
    }
    ...
    var dkSell = geSellValue(knifeData['Dragon knife']);
    var bestKnifeProfit = null;
    var bestKnifeName = '';
    if (F.KNIFE_ITEMS && dkSell > 0) {
      for (var name in knifeData) {
        if (name !== 'Dragon knife' && knifeData[name].low > 0) {
          var perProfit = dkSell - knifeData[name].low;
          var gross = perProfit * (F.KNIFE_BUY_LIMIT || 7000);
          var tax = geTaxEach(dkSell) * (F.KNIFE_BUY_LIMIT || 7000);
          var limitProfit = gross - tax;
          if (bestKnifeProfit == null || limitProfit > bestKnifeProfit) {
    ...
    var knifeLimit = F.KNIFE_BUY_LIMIT || 7000;
    var knifeTax = geTaxEach(dkSell) * knifeLimit;
    addTile('knife', 'Dragon Knife Cleaning', bestKnifeProfit != null ? bestKnifeProfit : 0, 'gp', { knives: knivesList, best_knife: bestKnifeName, tax: knifeTax }, 'Dragon knife');
    ...
    var mcSell = geSellValue(get(F.MULTICANNON_ID || 12863));
    var cannonHourly = 0;
    var partsProfit = null;
    var nulodionProfit = null;
    if (mcSell) {
      var cTax = geTaxEach(mcSell);
    ...
    addTile('cannon', 'Dwarven Multicannon', cannonHourly, 'gp/hr', {
      ...
      cannon_sell: mcSell, tax: geTaxEach(mcSell), parts_profit: partsProfit, nulodion_profit: nulodionProfit,
    ...
    var brewSellRaw = geSellValue(get(F.BANDITS_BREW_ID || 4627));
    var brewTax = geTaxEach(brewSellRaw);
    var brewSell = brewSellRaw - brewTax;
    var brewPer = brewSellRaw ? brewSell - (F.BANDIT_TRADER_GP || 750) : null;
    var banditHourly = brewPer != null ? brewPer * banditRate : 0;
    addTile('bandit', "Buying Bandit Brews", banditHourly, 'gp/hr', {
      ge_sell: brewSellRaw,
      ge_sell_after_tax: brewSell,
      tax: brewTax,
      profit_per: brewPer,
    ...
    // --- Sandworms (buy pack for 11,100 gp / 100 worms, sell worms on GE; profit per pack after 2% sell tax) ---
    var sandwormId = F.SANDWORM_ID || 13431;
    var sandwormPackId = F.SANDWORM_PACK_ID || 13432;
    var sandwormPackGp = F.SANDWORM_PACK_GP != null ? F.SANDWORM_PACK_GP : 11100;
    var sandwormQty = F.SANDWORM_PACK_QTY != null ? F.SANDWORM_PACK_QTY : 100;
    var sandwormRate = F.SANDWORM_HOURLY_RATE != null ? F.SANDWORM_HOURLY_RATE : 5400;
    var sandwormCostEach = sandwormQty ? sandwormPackGp / sandwormQty : 111;
    var sandwormRaw = geSellValue(get(sandwormId));
    var sandwormTax = geTaxEach(sandwormRaw);
    ...
    addTile('sandworms', 'Buying Sandworms', sandwormProfitPack, 'gp', {
      ...
      ge_sell: sandwormRaw,
      ge_sell_after_tax: sandwormAfterTax,
      tax: sandwormTax,
    ...
    var lockpickGeRaw = geSellValue(lockpickD);
    var lockpickTax = geTaxEach(lockpickGeRaw);
    ...
    var soulPriceRaw = geSellValue(get(F.SOUL_RUNE_ID || 566));
    var soulTax = geTaxEach(soulPriceRaw);
    ...
    addTile('soul_rune', 'Buying Soul Runes', soulProfit, 'gp', { ge_buy: soulPriceRaw, ge_buy_after_tax: soulPriceAfterTax, tax: soulTax, ge_buy_icon: iconById[String(F.SOUL_RUNE_ID || 566)] }, 'Soul rune');
    ...
    var profitPer = null;
    if (productValDisplay && costPer10) {
      var bdbTax = geTaxEach(productValDisplay);
      var revenuePerBottle = productValDisplay - bdbTax;
    ...
    addTile('dragonbreath', 'Making Bottled Dragonbreath', dragonbreathHourly, 'gp/hr', {
      ...
      product_after_tax: (typeof revenuePerBottle !== 'undefined') ? revenuePerBottle : null,
      tax: (typeof bdbTax !== 'undefined') ? bdbTax : null,
    ...
    var mithrilGeRaw = geSellValue(get(mithrilId));
    var mithrilTax = geTaxEach(mithrilGeRaw);
    ...
    addTile('mithril_seeds', 'Buying Mithril Seeds', mithrilHourly, 'gp/hr', {
      ...
      ge_price_after_tax: mithrilGeAfterTax,
      tax: mithrilTax,
  ...
  function processGemCutting(prices, iconById) {
    ...
    for (var i = 0; i < F.GEM_CUTTING_ITEMS.length; i++) {
      ...
      var tax = (cutValue != null && cutValue > 0) ? Math.min(Math.floor(cutValue * 0.02), maxTax) : 0;
      var profit = (uncutPrice != null && cutValue != null) ? (cutValue - tax) - uncutPrice : null;
  ...
  /** Overlay Flipwise known id/name pairs so charts work even if wiki casing/names drift. */
  function seedKnownIds(idByName) {
    var F = window.Flipwise;
    if (!F) return idByName || {};
    idByName = idByName || {};
    function add(id, name) {
      if (id == null || name == null || name === '') return;
      idByName[String(name)] = id;
    }
    function addList(list) {
      ...
      for (var i = 0; i < list.length; i++) {
        var it = list[i];
        if (!it) continue;
        if (it.id != null && it.name) add(it.id, it.name);
        if (it.output_id != null && it.output_name) add(it.output_id, it.output_name);
        if (it.uncut_id != null && it.uncut_name) add(it.uncut_id, it.uncut_name);
        if (it.cut_id != null && it.cut_name) add(it.cut_id, it.cut_name);
        if (it.item_id != null && it.display_name) add(it.item_id, it.display_name);
        if (it.id4 != null && it.name) add(it.id4, it.name);
        if (it.inputs) {
          for (var j = 0; j < it.inputs.length; j++) {
            if (it.inputs[j]) add(it.inputs[j].id, it.inputs[j].name);
          }
        }
      }
    }
    addList(F.FLIP_ITEMS);
    addList(F.THIRD_AGE_ITEMS);
    addList(F.HIGH_VOLUME_ITEMS);
    addList(F.HERBLORE_ITEMS);
    addList(F.KNIFE_ITEMS);
    addList(F.CANNON_ITEMS);
    addList(F.ODIUM_SHARD_ITEMS);
    addList(F.MALEDICTION_SHARD_ITEMS);
    addList(F.TREE_SAPLING_ITEMS);
    addList(F.GEM_CUTTING_ITEMS);
    ...
    add(22486, 'Scythe of Vitur');
    add(27277, "Tumeken's Shadow");
    add(27641, 'Saturated Heart');
    add(23997, 'Blade of saeldor');
    add(25862, 'Bow of faerdhinen');
    add(31115, 'Eye of ayak (uncharged)');
    if (F.THIRD_AGE_ITEMS) {
      for (var t = 0; t < F.THIRD_AGE_ITEMS.length; t++) {
        var ta = F.THIRD_AGE_ITEMS[t];
        if (ta && ta.id != null && ta.name) add(ta.id, String(ta.name).replace('3rd Age', '3rd age'));
      }
    }
    return idByName;
  }
  ...
  function refresh() {
    return fetchParallel().then(function(data) {
      ...
      return {
        ...
        iconById: iconById,
        idByName: seedKnownIds(data.idByName || {}),
        itemData: itemData,
  ...
  function fetchTimeseries(itemId, lookback) {
    ...
    if (lookback === '5m') {
      var url5m = API_BASE + '/timeseries?timestep=5m&id=' + encodeURIComponent(itemId);
      return get(url5m).then(function(j) {
        var data = j.data || [];
        return {
          data: data,
          itemId: j.itemId,
          startTimestamp: data.length ? data[0].timestamp : null,
          endTimestamp: data.length ? data[data.length - 1].timestamp : null,
          timestep: 300
        };
      });
    }

    var url = TIMESERIES_BASE + '/timeseries?id=' + encodeURIComponent(itemId) +
